...
const COINS = [
  ...
  { label: "BNB/USD", binance: "BNBUSDT", coinbase: null, kraken: null, okx: "BNB-USDT", bybit: "BNBUSDT", gecko: "binancecoin" },
...
const parseKraken = d => { const r = d.result || {}; const key = Object.keys(r).find(k => k !== "last"); if (!key) throw new Error("kraken parse"); return r[key].map(k => ({ time: +k[0] * 1000, high: +k[2], low: +k[3], close: +k[4], volume: +k[6] })); };
const parseOKX = d => (d.data || []).map(k => ({ time: +k[0], high: +k[2], low: +k[3], close: +k[4], volume: +k[5] }));
const parseBybit = d => ((d.result && d.result.list) || []).map(k => ({ time: +k[0], high: +k[2], low: +k[3], close: +k[4], volume: +k[5] }));
...
const parseYahoo = d => {
  ...
  return r.timestamp.map((t, i) => ({ time: t * 1000, high: q.high[i], low: q.low[i], close: q.close[i], volume: q.volume ? q.volume[i] : null }))
...
function sourcesFor(coin) {
  if (coin.fx) return [{ name: "YahooFX", noVol: true, run: () => fetchYahoo(coin.fx) }];
  const list = [];
  if (coin.coinbase) list.push({ name: "Coinbase", noVol: false, run: async () => {    const r = await timeoutFetch(`https://api.exchange.coinbase.com/products/${coin.coinbase}/candles?granularity=300`);
    if (!r.ok) throw new Error("cb " + r.status); return parseCoinbase(await r.json()); }});
  list.push({ name: "OKX", noVol: false, run: async () => {
    const r = await timeoutFetch(`https://www.okx.com/api/v5/market/candles?instId=${coin.okx}&bar=5m&limit=100`);
    if (!r.ok) throw new Error("okx " + r.status); return parseOKX(await r.json()); }});
  list.push({ name: "Bybit", noVol: false, run: async () => {
    const r = await timeoutFetch(`https://api.bybit.com/v5/market/kline?category=spot&symbol=${coin.bybit}&interval=5&limit=100`);
...
function analyze(rows, allowNoVolume, forex) {
  ...
  const highs = rows.map(r => Number(r.high));  const lows = rows.map(r => Number(r.low));
  ...
  const volatilityOk = forex
    ? (atrPct >= 0.008 && atrPct <= 0.6)
    : (atrPct >= 0.04 && atrPct <= 3.0);
...
async function checkCoin(coin) {
  try {
    const win = await Promise.any(sourcesFor(coin).map(async src => {
      ...
      if (Date.now() - rows[rows.length - 1].time > 20 * 60000) throw new Error("stale");
      return { src, rows };
    }));
    return { symbol: coin.label, source: win.src.name, ...analyze(win.rows, win.src.noVol, coin.forex) };
  } catch (e) {
...
async function fetchPriceAt(coin, tsMs) {
  try {
    const rows = await Promise.any(sourcesFor(coin).map(async src => {
      const r = sortRows(await src.run());
      if (!r || r.length < 10) throw new Error("short");
      return r;
    }));
...
async function sendSignalEmail(newSignals, allGood) {
  ...
  const now = new Date();
  const blocks = (newSignals.length ? newSignals : allGood).map(s => {    const e = new Date(s.entryTime);
...
async function run({ email = false, force = false, checkOnly = false } = {}) {
  ...
  const assets = await Promise.all(COINS.map(checkCoin));  const minConf = parseInt(process.env.MIN_CONFIDENCE || "75", 10);
