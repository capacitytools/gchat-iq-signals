...
/* THE FIX: always sort candles old → new, no matter what order the exchange sends */
...
async function run({ email = false, force = false, checkOnly = false } = {}) {
  ...
  const { data, sha } = await store.readJSON(file, { signals: [], meta: {} });  const signals = data.signals || [];
